  ...
  class Map
    class SheetMaker
      ...
      def make_sheet(pivot_block : Block)
        ...
        Renderer.render_sheets_channel.send({@render_grid.render_type, receive_sheet_channel, printables})
      ...
      def create_sheet(pivot_block : Block)
        ...
        col, row = grid.sheet_for(pivot_block)
        sheet = grid.create_sheet(col, row, pixels)
        top, left, right, bottom = {@top, @left, @right, @bottom}
        raise SheetMadeEmpty.new("Boundaries are not present for sheet t,l,r,b: #{top}, #{left}, #{right}, #{bottom}") unless top && left && right && bottom
        sheet.top = top
        sheet.left = left
        sheet.right = right
        sheet.bottom = bottom
        Log.debug { "Created Sheet: #{col}, #{row}" }
      end
          ...
          0_u8.upto(screen_cols).each do |col|
            ...
            @layers.each do |id|
              ...
              layer.chunks.each do |chunk|
                ...
                rect = SDL::Rect.new((col.to_i * @map.tile_width).to_i, (row.to_i * @map.tile_height).to_i, @map.tile_width.to_i, @map.tile_height.to_i)
