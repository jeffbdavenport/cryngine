module Cryngine
  module Map
    class Sheet
      ...
      getter texture : Pointer(LibSDL::Texture)
      @texture = uninitialized Pointer(LibSDL::Texture)
      ...
      def self.initialize(window, player_block)
        @@width = window.width
        @@height = window.height
        @@offset_x = player_block.real_x
        @@offset_y = player_block.real_y
      end
      ...
      def initialize(@col, @row, @texture)
      ...
      def self.sheet_col(real_x)
        ...
        width = (Map::Player.unscaled_pixels_col * 2)
      ...
      def print_x_corner_from(real_x)
        ...
        x -= offx
        (col.to_i64 * self.class.scaled_width) - Map::Player.unscaled_pixels_col - x + Map::Player.scaled_pixels_col - ((Map.scaled_tile_width) / 2).to_i
      end
      ...
      def print_y_corner_from(real_y)
        ...
        y -= offy
        (row.to_i64 * self.class.scaled_height) - Map::Player.unscaled_pixels_row - y + Map::Player.scaled_pixels_row - ((Map.scaled_tile_width) / 2).to_i + Map::Player.player_height
      end
